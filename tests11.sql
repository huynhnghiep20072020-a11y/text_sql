...
begin 
select *from users where id= p_user_id and p_user_id is not null;

end//
delimiter ;
call get_user_info(4);
select *from users ;

-- b2
delimiter //
create procedure add_new_product(product_name varchar(100),price decimal(10,2),tock int )
begin

if price <= 0 then 
	select "gia phia lon hon 0" "";
else 
insert into product (name,price,stock) value 
...
delimiter ;
call add_new_product("bang trang","1000000");

-- B3 
delimiter //
create procedure calculate_discount_price(product_id int, discount tinyint)
begin
select ceil((select price from products where id = product_id and product_id is not null)*(1-discount/100))price;

end //
delimiter ;
call calulate_discount_price(8,30);
...
-- Thêm 1 tham số OUT là p_TotalRevenue (DECIMAL). SP sẽ tính tổng tiền (TotalPrice) 
-- từ bảng Orders trong tháng và năm đó, rồi gán kết quả vào biến OUT.
...
CREATE PROCEDURE GetTotalRevenueByMonth(
    IN p_Month INT,
    IN p_Year INT,
    OUT p_TotalRevenue DECIMAL(18,2)
)
BEGIN
    -- Sử dụng SELECT ... INTO để gán kết quả SUM vào tham số OUT
    SELECT SUM(TotalPrice) INTO p_TotalRevenue
    FROM Orders
    WHERE MONTH(OrderDate) = p_Month 
      AND YEAR(OrderDate) = p_Year;

    -- Lưu ý: Nếu tháng đó không có đơn hàng nào, p_TotalRevenue sẽ nhận giá trị NULL.
    -- Để chuyên nghiệp hơn, em có thể dùng hàm IFNULL(SUM(...), 0) để trả về 0 thay vì NULL.
END //

DELIMITER ;
-- 1. Khai báo một biến phiên (Session Variable) để hứng kết quả
SET @total = 0;

-- 2. Gọi SP (Ví dụ lấy doanh thu tháng 5 năm 2026)
CALL GetTotalRevenueByMonth(5, 2026, @total);

-- 3. In biến đó ra để xem kết quả
SELECT @total AS Total_Revenue_May_2026;

-- b5
delimiter //
create procedure search_product_by_name(p_keyword varchar(50))
begin 
select *from products where name like concat("%"+p_keyword +"%");
end//
delimiter ;
call search_product_by_name("sony");

-- b6
DROP PROCEDURE IF EXISTS update_user_tier;
DELIMITER //

CREATE PROCEDURE update_user_tier(IN p_user_id INT)
BEGIN
    -- Khai báo biến cục bộ (Anh đổi tên thành v_user_total_spent cho đúng chuẩn tiền tố v_ của biến)
    DECLARE v_user_total_spent DECIMAL(18,2) DEFAULT 0;

    -- Lấy tổng chi tiêu của user nạp vào biến
    SELECT total_spent INTO p_user_total_spent 
    FROM users 
    WHERE id = p_user_id;

    -- Rẽ nhánh phân hạng khách hàng
    IF p_user_total_spent < 1000000 THEN 
        SELECT 'Khách hàng thường' AS user_rank;
        
    ELSEIF p_user_total_spent < 5000000 THEN 
        SELECT 'Khách hàng bạc' AS user_rank;
        
    ELSEIF p_user_total_spent < 10000000 THEN 
        SELECT 'Khách hàng vàng' AS user_rank;
        
    ELSE 
        SELECT 'Khách hàng kim cương' AS user_rank;
        
    END IF;
END //

DELIMITER ;
CALL update_user_tier(3);
